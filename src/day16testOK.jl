...
# min_dist = minimum(dist[(goal..., dir_i)] for dir_i in axes(directions, 1))  # minimal dist to goal regardless of direction
# goal_dirs = [dir_i for dir_i in axes(directions, 1) if dist[(goal..., dir_i)] == min_dist]  # final firections that minimize the dist to the goal

# equivalent to above
f(dir_i) = dist[(goal..., dir_i)]
findall(==(minimum(f, axes(directions, 1))) ∘ f, axes(directions, 1))
...
argmin(dir_i -> dist[(goal..., dir_i)], axes(directions, 1))
findall(==(minimum(dir_i -> dist[(goal..., dir_i)], axes(directions, 1))), axes(directions, 1))
