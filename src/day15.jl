...
function can_move!(lobstacles_to_move, robstacles_to_move, iswall, is_lobstacle, is_robstacle, obstacle_pos, move, part2)
    if iswall[obstacle_pos...] return false end
    ...
    for (obs_to_move, is_obs, sibling) in ((lobstacles_to_move, is_lobstacle, lsibling), (robstacles_to_move, is_robstacle, rsibling))
        ...
        return all(can_move!(lobstacles_to_move, robstacles_to_move, iswall, is_lobstacle, is_robstacle, obstacle_pos .+ offset, move, part2) for offset in (move, sibling))
    end
    return true
end

"""return true if the robot can step, and push the obstacles that need to move to obstacles_to_move"""
function can_step!(lobstacles_to_move, robstacles_to_move, iswall, is_lobstacle, is_robstacle, pos, move, part2)
    return can_move!(lobstacles_to_move, robstacles_to_move, iswall, is_lobstacle, is_robstacle, pos + move, move, part2)
end

function room_to_string(iswall, is_lobstacle, is_robstacle, pos, part2)
    str = fill('.', size(iswall))
    str[iswall] .= '#'
    if !part2
        str[is_lobstacle] .= 'O'
    else
...
function solve(lines, part2)
    ...
    @debug "Initial state:\n" * room_to_string(iswall, is_lobstacle, is_robstacle, pos, part2)
    ...
    for (i, char_move) in enumerate(moves)
        ...
        can_move = can_step!(lobstacles_to_move, robstacles_to_move, iswall, is_lobstacle, is_robstacle, pos, move, part2)
        if can_move
            pos .+= move
        ...
        if Logging.min_enabled_level(current_logger()) <= Logging.Debug
            ...
            if iswall[pos...] @debug "walking in walls at step $i"; break end
            ...
            if any(is_lobstacle .& iswall) @debug "lobstacles on wall at step $i"; break end
            if any(is_robstacle .& iswall) @debug "robstacles on wall at step $i"; break end
