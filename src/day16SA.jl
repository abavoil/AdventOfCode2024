...
function parse_maze(lines, directions)
    ...
    is_wall = mat .== '#'
    start = Pos(findfirst(mat .== 'S').I)
    goal = Pos(findfirst(mat .== 'E').I)
    costs = Dict{State, Array{Tuple{State, Int}}}()  # state -> (next_state, move_cost)
    for pos in Pos.(getfield.(findall(.!is_wall), :I))
        for dir in 1:4
...
# lines = empty_maze(40)
directions = SVector.([(1, 0), (0, 1), (-1, 0), (0, -1)])
@btime parse_maze($lines, $directions) # 4.844 ms (81200 allocations: 16.30 MiB)
mat, start, goal, costs = parse_maze(lines, directions)
@info "[Maze]\n" *arr2str(mat)

dists_from_origin = infty * ones(Int, size(mat)..., size(directions, 1))
@time explore!(dists_from_origin, costs, goal, 0, start, 2) # 6.300561 seconds (109.83 M allocations: 4.910 GiB, 2.32% gc time)
